...
-- solution:
select product_name, sub.product_id, order_id, order_date
from(select *,
rank() over (partition by product_id order by order_date desc) as rank_order
from orders) as sub
left join products
on products.product_id = sub.product_id
where rank_order = 1
