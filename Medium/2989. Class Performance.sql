...
-- solution:
with cte as (select student_id, student_name, (assignment1+assignment2+assignment3) as total
from Scores)

select (select max(total) from cte) - (select min(total) from cte) as difference_in_score
from cte
limit 1
